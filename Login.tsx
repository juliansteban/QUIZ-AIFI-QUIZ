...
import { LogIn, GraduationCap, Presentation } from 'lucide-react';

export default function Login({ onLogin, isAdminRoute = false }: { onLogin: (user: any, token: string) => void, isAdminRoute?: boolean }) {
  ...
  return (
    <div className="max-w-md mx-auto mt-20 glass-card p-8">
      ...
      <h2 className="text-2xl font-bold text-center mb-6 text-white">
        Iniciar Sesión como {role === 'student' ? 'Estudiante' : 'Profesor'}
      </h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          Entrar como {role === 'student' ? 'Estudiante' : 'Profesor'}
        </button>
      </form>
      
      {!isAdminRoute && (
        <p className="text-center mt-6 text-sm text-white/60">
          ¿No tienes cuenta? <Link to="/register" className="hover:underline text-indigo-400">Regístrate</Link>
        </p>
      )}
    </div>
  );
